...
export function WeeklyStreaks() {
  ...
  return (
    <div className={`rounded-2xl p-6 shadow-sm border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>WEEKLY STREAKS</h3>
          ...
          <div
            ...
            className={`
              ...
              ${day.completed 
                ...
                : theme === 'dark' ? 'bg-gray-700 text-gray-400' : 'bg-gray-50 text-gray-400'}
      ...
      <div className={`mt-4 pt-4 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-100'}`}>
        <div className="flex justify-between text-sm">
          <span className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}>Longest streak</span>
          <span className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{streak?.longestStreak || 0} days</span>
        </div>
      </div>
    </div>
  );
